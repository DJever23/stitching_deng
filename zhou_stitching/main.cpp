...
//#define RESIZE_WIDTH 1600
//#define RESIZE_HEIGHT 1200
...
struct GradientData { //存储各个方向梯度信息
	cv::Mat dx;
	cv::Mat dy;
	cv::Mat dxdy;
	cv::Mat angle;
...
void getGradient(cv::Mat& src, cv::Mat& dst, int dx, int dy) {
	...
	// Convert our images to float 32 bit precision 图片转换为灰度图
	...
	cv::Mat& img2)
{
	...
	for (int i = 0; i < inlierSet.count; ++i) {
		...
		line(inlierImg, p1, p2, color, 1);
	}
	//
	cv::imshow("initial_img", inlierImg);
	cv::imwrite("Match.png", inlierImg);
    //
}
...
void drawKeypointDescs(cv::Mat&dst, std::vector<KeypointDescriptor>& interestPoints) {
	...
	for (int i = 0; i < interestPoints.size(); ++i) {
		circle(dst, Point(interestPoints[i].keypoint.pt.x, interestPoints[i].keypoint.pt.y), interestPoints[i].keypoint.size * 0.15, color, 2);
...
int main(int argc, char** argv)
{
	...
	//std::string files[] = { "./imgs/project_images/MelakwaLake1.png", "./imgs/project_images/MelakwaLake2.png" };
	//std::string files[] = { "imgs/project_images/MelakwaLake3.png", "imgs/project_images/MelakwaLake4.png" };
//	std::string files[] = { "./A01.jpg", "./A02.jpg" };
	//std::string files[] = { "MelakwaLake1.png","MelakwaLake2.png"};
	std::string files[] = { "1.jpg","2.jpg"};
//	Mat input1 = imread(files[0], CV_LOAD_IMAGE_COLOR);
//	Mat input2 = imread(files[1], CV_LOAD_IMAGE_COLOR);
	Mat input1 = imread(files[0],1);
	Mat input2 = imread(files[1],1);
	resize(input1, input1, Size(input1.cols / 4, input1.rows / 4),(0, 0), (0, 0), cv::INTER_AREA);
	resize(input2, input2, Size(input2.cols / 4, input2.rows / 4),(0, 0), (0, 0), cv::INTER_AREA);
	//cv::resize(input1,input1,cv::Size(RESIZE_WIDTH, RESIZE_HEIGHT), (0, 0), (0, 0), cv::INTER_AREA);
	//cv::resize(input2,input2,cv::Size(RESIZE_WIDTH, RESIZE_HEIGHT), (0, 0), (0, 0), cv::INTER_AREA);
	//imwrite("resize1.jpg",input1);
	//imwrite("resize2.jpg",input2);
	Mat output;
	...
	imshow("Stitch", stichedImage);
	imwrite("afterStitch.jpg", stichedImage);
