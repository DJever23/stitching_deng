...
int main()
{
	...
	//cv::resize(a,a,cv::Size(1600,1200), (0, 0), (0, 0), cv::INTER_AREA);
	//cv::resize(b,b,cv::Size(1600,1200), (0, 0), (0, 0), cv::INTER_AREA);
	...
	//surf->detectAndCompute(b, Mat(), key2, d);//这个Mat行数为特征点的个数，列数为每个特征向量的尺寸，SURF是64（维）
 	sift->detectAndCompute(a, Mat(), key1, c);
	sift->detectAndCompute(b, Mat(), key2, d);
